# M5 TA1 ownership regression gate. TI's USB_init() uses TA1 to detect the
...
if ($mainEntryIndex -lt 0) {
    throw 'M5 timer ownership: main entry point is missing'
}
...
if (($usbSetupIndex -lt 0) -or ($timerInitIndex -lt 0)) {
    throw 'M5 timer ownership: USB or application timer initialization is missing'
}
if ($timerInitIndex -lt $usbSetupIndex) {
    throw 'M5 timer ownership: TA1 must be initialized after TI USB_setup() releases it'
}

Write-Host 'M5 TA1 ownership audit: PASS'
