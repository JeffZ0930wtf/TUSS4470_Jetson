# Programs only the small-RAM, no-acquisition M3 startup diagnostic image.
param(
...
$image = Join-Path $repositoryRoot `
    'firmware\build\m3-small-ram-diagnostic\usac-m3-small-ram-diagnostic-no-acquisition.elf'
...
foreach ($required in @($image, $dslite, $targetConfig, $settings)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "required small-RAM diagnostic flash input is missing: $required"
    }
}
...
Write-Host 'M3 small-RAM diagnostic flash: external VPWR confirmed OFF.'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "TI DSLite small-RAM diagnostic flash/verify failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
...
Write-Host 'M3 small-RAM diagnostic flash and verify: PASS'
