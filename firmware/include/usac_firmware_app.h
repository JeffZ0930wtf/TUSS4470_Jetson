/* firmware application state and command dispatcher. Hardware actions are injected
 * as callbacks so simulator tests can verify state transitions and failure
 * closure without accessing the real board. */
#ifndef USAC_FIRMWARE_APP_H
#define USAC_FIRMWARE_APP_H
...
typedef uint8_t (*usac_capture_fn)(
    void *context,
...
typedef struct {
    ...
    usac_capture_report_t capture_report;
...
#ifdef USAC_ENABLE_ACQUISITION
    usac_capture_schedule_t periodic;
