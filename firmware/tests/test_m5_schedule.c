...
static int test_periodic_schedule_is_bounded_and_finite(void)
{
    ...
    CHECK(usac_m5_schedule_start(
              &schedule, schedule_id, 99999ul, 2ul, 3000u) ==
          USAC_M5_SCHEDULE_INVALID);
    CHECK(usac_m5_schedule_start(
              &schedule, schedule_id, 100000ul, 2ul, 3000u) ==
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_advance(&schedule, 99999ul) ==
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_claim_due(&schedule) == 0u);
    CHECK(usac_m5_schedule_advance(&schedule, 1ul) ==
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_claim_due(&schedule) == 1u);
    CHECK(usac_m5_schedule_advance(&schedule, 200000ul) ==
          USAC_M5_SCHEDULE_OK);
...
static int test_lease_expires_and_renewal_is_monotonic(void)
{
    ...
    CHECK(usac_m5_schedule_start(
          ...
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_renew(
              &schedule, other_id, 1ul, 1000u) ==
          USAC_M5_SCHEDULE_CONFLICT);
    CHECK(usac_m5_schedule_renew(
    ...
    CHECK(usac_m5_schedule_renew(
              &schedule, schedule_id, 1ul, 2000u) ==
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_lease_remaining_ms(&schedule) == 2000u);
    CHECK(usac_m5_schedule_advance(&schedule, 1999999ul) ==
          USAC_M5_SCHEDULE_OK);
    CHECK(usac_m5_schedule_advance(&schedule, 1ul) ==
...
static int test_slave_sync_timeout_never_expires_early(void)
{
    ...
    CHECK(usac_m5_sync_timeout_guarded_quanta(0ul) == 0u);
    CHECK(usac_m5_sync_timeout_guarded_quanta(1ul) == 2u);
    CHECK(usac_m5_sync_timeout_guarded_quanta(1000ul) == 104u);
    CHECK(usac_m5_sync_timeout_guarded_quanta(5000ul) == 513u);
    CHECK(usac_m5_sync_timeout_guarded_quanta(60000ul) == 6145u);
    return 0;
}
