...
#define USAC_QUALITY_EVENT_OVERFLOW 0x00000040ul
#define USAC_QUALITY_EVENT_TIME_AMBIGUOUS 0x00000080ul
...
static uint8_t capture_active;
static uint8_t capture_aux_flags;
static uint8_t capture_trigger_source;
static uint32_t capture_sync_timeout_ms;
static usac_burst_plan_t capture_burst_plan;
...
static usac_capture_report_t *capture_event_report;
#endif
    ...
    uint16_t cctl)
{
    ...
    if ((cctl & COV) != 0u) {
        capture_event_report->quality_flags |=
            USAC_QUALITY_EVENT_TIME_AMBIGUOUS;
    }
    if (((TA0CTL & TAIFG) != 0u) && (captured_tick < 0x8000u)) {
        ++epoch;
    }
    frame_offset = (epoch << 16) | captured_tick;
    if (capture_event_report->event_count >= USAC_MAX_CAPTURE_EVENTS) {
        capture_event_report->quality_flags |= USAC_QUALITY_EVENT_OVERFLOW;
        return;
    }
...
static uint8_t wait_for_slave_sync(uint32_t timeout_ms)
{
    ...
    uint16_t timeout_quanta =
        usac_sync_timeout_guarded_quanta(timeout_ms);
...
static void configure_burst_outputs(const usac_burst_plan_t *plan)
{
    ...
    TA2CCTL1 = 0u;
    TA2CCTL2 = 0u;
    if ((plan->action_flags & USAC_BURST_USE_IO1_TIMER) != 0u) {
...
static void start_burst_outputs(void)
{
    if ((capture_burst_plan.action_flags &
         USAC_BURST_USE_IO1_ENABLE) != 0u) {
        P2OUT &= (uint8_t)~TUSS_IO1_BIT;
    }
    if ((capture_burst_plan.action_flags &
         USAC_BURST_USE_IO1_TIMER) != 0u) {
...
#endif
#ifndef USAC_ADC_DMA_DIAGNOSTIC
    ...
    usac_loopback_report_t *report)
{
    ...
    for (index = 0u; index < USAC_LOOPBACK_EDGE_COUNT; ++index) {
        ...
        if ((report->result_flags & USAC_LOOPBACK_TIMEOUT) != 0u) {
            break;
        }
        if ((TA0CCTL4 & COV) != 0u) {
    ...
    if ((tuss4470_force_safe(bus) != TUSS4470_CONFIG_OK) ||
        ...
        (snapshot_is_safe(
             ...
             report->post_tof_config,
             report->post_vdrv_ctrl) == 0u)) {
        report->result_flags |= USAC_LOOPBACK_PREPOST_SAFETY_FAILED;
    }
...
static uint8_t prepare_adc_dma_capture_path(void)
{
    ...
    __data16_write_addr(
        (uintptr_t)&DMA0DA, (uintptr_t)g_usac_capture_waveform);
    DMA0SZ = USAC_CAPTURE_SAMPLE_COUNT;
...
#else
    DMA1SZ = USAC_CAPTURE_DEFAULT_PRETRIGGER_COUNT;
    ...
    usac_capture_report_t *report)
{
...
#ifdef USAC_ENABLE_ACQUISITION
    if (capture_active == 0u) {
        capture_pretrigger_target = USAC_CAPTURE_DEFAULT_PRETRIGGER_COUNT;
    ...
    if (adc_completed != 0u) {
        g_usac_capture_waveform[0] = 0xFFFFu;
        DMA0CTL = 0u;
        DMACTL0 = DMA0TSEL_0;
        __data16_write_addr((uintptr_t)&DMA0SA, (uintptr_t)&adc_sample);
        __data16_write_addr(
            (uintptr_t)&DMA0DA, (uintptr_t)&g_usac_capture_waveform[0]);
        DMA0SZ = 1u;
        ...
        if (((DMA0CTL & DMAIFG) != 0u) &&
            (g_usac_capture_waveform[0] == adc_sample)) {
            dma_completed = 1u;
        } else {
            report->timed_out = 1u;
        }
    ...
    report->dma_remaining = g_usac_capture_waveform[0];
    ...
    report->diagnostic_flags =
        ((__get_SR_register() & GIE) != 0u) ?
            USAC_CAPTURE_DIAG_GIE_AT_ENTRY : 0u;
    report->diagnostic_timer_tick = 0u;
#endif
#ifdef USAC_ENABLE_ACQUISITION
    if (capture_active != 0u) {
        capture_events_arm(
            capture_aux_flags, sample_interval_ticks, report);
    } else {
        report->out3_start_level = 0xFFu;
        report->out4_start_level = 0xFFu;
    }
...
#ifdef USAC_ENABLE_ACQUISITION
    if (capture_active != 0u) {
        configure_burst_outputs(&capture_burst_plan);
    } else
#endif
    {
        P2OUT |= TUSS_IO2_BIT;
...
#ifdef USAC_ENABLE_ACQUISITION
    if ((capture_active != 0u) &&
        (capture_trigger_source == 1u) &&
        (wait_for_slave_sync(capture_sync_timeout_ms) == 0u)) {
    ...
    if (capture_complete == 0u) {
        ...
        if ((TB0CTL & MC_3) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_TB0_RUNNING;
        }
        if ((ADC12CTL0 & ADC12ENC) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_ADC_ENABLED;
        }
        if ((ADC12IFG & ADC12IFG0) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_ADC_IFG0;
        }
        if ((DMA0CTL & DMAEN) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_DMA0_ENABLED;
        }
        if ((DMA0CTL & DMAIFG) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_DMA0_IFG;
        }
        if ((DMA1CTL & DMAIFG) != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_DMA1_IFG;
        }
        if (capture_burst_complete != 0u) {
            report->diagnostic_flags |= USAC_CAPTURE_DIAG_BURST_COMPLETE;
        }
#endif
    }
    stop_capture_hardware();
    if (capture_dma0_completed != 0u) {
        ...
        report->captured_samples =
            (uint16_t)(USAC_CAPTURE_SAMPLE_COUNT - DMA0SZ);
    }
...
#ifdef USAC_ENABLE_ACQUISITION
    if (capture_active != 0u) {
        return usac_capture_report_is_complete(
            report, capture_pretrigger_target);
    }
#endif
    return usac_capture_report_is_default_complete(report);
#endif
}
...
#else
            capture_trigger_sample_index = USAC_CAPTURE_DEFAULT_PRETRIGGER_COUNT;
            P2SEL |= TUSS_IO2_BIT;
            TA2CTL = TASSEL_2 | MC_1 | TACLR;
#endif
            break;
    ...
    usac_capture_report_t *report)
{
    ...
    if ((bus == 0) || (bus->read == 0) || (bus->write == 0) ||
        ...
        (config->sample_count != USAC_CAPTURE_SAMPLE_COUNT) ||
        (config->pretrigger_count >= USAC_CAPTURE_SAMPLE_COUNT)) {
        return 0u;
    }
    io_mode = (uint8_t)(config->profile.registers[4].value & 0x03u);
    pulse_count = (uint8_t)(config->profile.registers[8].value & 0x3Fu);
    posttrigger_ticks =
        (uint32_t)(USAC_CAPTURE_SAMPLE_COUNT - config->pretrigger_count) *
        config->profile.sample_interval_ticks;
    if (usac_burst_plan_build(
            io_mode,
            pulse_count,
            config->profile.burst_period_ticks,
            posttrigger_ticks,
            &capture_burst_plan) == 0u) {
        return 0u;
    }
    ...
    capture_aux_flags = config->aux_flags;
    capture_trigger_source = trigger_source;
    capture_sync_timeout_ms = sync_timeout_ms;
    capture_active = 1u;
    ...
    capture_active = 0u;
    capture_aux_flags = 0u;
    capture_trigger_source = 0u;
    capture_sync_timeout_ms = 0ul;
