    ...
    usac_burst_plan_t *plan)
{
    ...
    plan->action_flags = USAC_BURST_USE_IO2_TIMER;
    if (io_mode == 0u) {
        plan->action_flags |= USAC_BURST_USE_CMD_TRIGGER;
    } else if (io_mode == 1u) {
        plan->action_flags |= USAC_BURST_USE_IO1_ENABLE;
    } else if (io_mode == 2u) {
        ...
        plan->action_flags |= USAC_BURST_USE_IO1_TIMER |
            USAC_BURST_NONOVERLAP;
    }
    return 1u;
}
