...
Implementation and acceptance completed on 2026-09-11. The immutable artifact-build candidate is `3642f45905f1ec3113d89d693ee827df1aa71e74`; Windows, Jetson, ARM64/AMD64 container, firmware-equivalence, and one bounded Pulse=1 physical-capture evidence are indexed by `docs/release/v1.0.0-manifest.md`.

One controlled deviation was accepted during the final merged-main gate: `.gitattributes` was updated after the candidate to force `requirements-container.lock.txt` to LF. This fixes byte-exact lock regeneration in fresh Windows checkouts. The tracked lock bytes, dependency resolution, runtime/build source, firmware, protocol, database schema, and hardware-control behavior did not change. The Windows repository `.venv` was subsequently synchronized from `uv.lock` and verified to expose only the four V1 public commands.
...
- Freeze all runtime/build inputs in `V1_CANDIDATE_SHA` before building candidate artifacts. Later acceptance commits may change only `docs/`, except for a separately documented repository-checkout rule that does not alter tracked artifact bytes or runtime/build inputs. The final release is valid only when an explicit path-limited Git diff proves the documented boundary.
...
- [x] **Step 1: Refresh and verify the exact remote starting state**
...
- [x] **Step 3: Build the accepted pre-rename production firmware without hardware access**
...
- [x] **Step 4: Save baseline artifacts outside Git**
...
- [x] **Step 6: Commit the baseline record**
...
- [x] **Step 3: Rename the modules and update imports**
...
- [x] **Step 4: Make real-hardware invocation explicit in tests and documentation fixtures**
...
- [x] **Step 5: Verify complete offline export byte identity**
...
- [x] **Step 1: Add failing tests for the final Bridge CLI**
...
- [x] **Step 2: Add or identify coverage for every retained delivery invariant**
...
- [x] **Step 3: Run the targeted tests and confirm the new CLI test fails**

Expected: failure because `serve` is still a required subcommand.

- [x] **Step 4: Move the active ID helper and simplify `bridge_cli.py`**
...
- [x] **Step 5: Delete superseded installed utilities only after migrated tests pass**
...
- [x] **Step 6: Run Bridge, spool, delivery, reconnect, and project-layout tests**

Expected: all retained behavior passes; importing `bridge_cli` no longer imports milestone modules.

- [x] **Step 7: Commit the Bridge consolidation**
...
- [x] **Step 1: Change the Node test to require neutral asset names**
...
- [x] **Step 2: Run Node and API asset tests to confirm failure**

Expected: failure on the old filenames/routes.

- [x] **Step 3: Rename assets and update references without changing UI logic**
...
- [x] **Step 4: Run Node, API, and server tests**

Expected: all Web behavior tests pass with the neutral asset paths.

- [x] **Step 5: Commit the Web rename**
...
- [x] **Step 1: Inventory each existing firmware check by behavior**
...
- [x] **Step 2: Mark obsolete artifacts by reason, not by milestone name**
...
- [x] **Step 3: Review the matrix for gaps**
...
- [x] **Step 4: Commit the migration matrix**
...
- [x] **Step 1: Add a filename and identifier hygiene test**
...
- [x] **Step 2: Run the hygiene test and confirm failure**

Expected: it reports the current M2/M3/M5 files and identifiers.

- [x] **Step 3: Rename files with `git mv` and update one responsibility group at a time**
...
- [x] **Step 4: Rename first-party identifiers consistently**
...
- [x] **Step 5: Build the renamed formal firmware**
...
- [x] **Step 6: Compare post-rename loadable content against Task 1**
...
- [x] **Step 7: Run all retained firmware unit and static safety checks**

Expected: every check in the Task 5 migration matrix passes.

- [x] **Step 8: Commit the firmware source rename**
...
- [x] **Step 1: Update project-layout tests for the final script set**
...
- [x] **Step 2: Run the layout test and confirm failure**

Expected: current milestone wrappers and image tags violate the final layout.

- [x] **Step 3: Build one formal firmware script**
...
- [x] **Step 4: Rename the controlled flash script**
...
- [x] **Step 5: Rename retained functional checks and prune obsolete daily builds**

Apply the Task 5 matrix. Do not remove a check solely because its old filename contains M2/M3/M5.

- [x] **Step 6: Update the Windows aggregate gate's firmware section**
...
- [x] **Step 7: Run the renamed firmware checks individually**
...
- [x] **Step 8: Reconfirm firmware equivalence after final build-script consolidation**
...
- [x] **Step 9: Commit build and gate consolidation**
...
- [x] **Step 2: Export one hash-locked container dependency set from `uv.lock`**
...
- [x] **Step 3: Update Docker and Compose references**
...
- [x] **Step 4: Complete deployment and aggregate-gate references**
...
- [x] **Step 5: Rewrite current Windows and Jetson operating instructions**
...
- [x] **Step 6: Run deployment/configuration checks without building an image**
...
- [x] **Step 7: Commit deployment migration**
...
- [x] **Step 1: Create the V1 acceptance and limitations documents**
...
- [x] **Step 2: Move completed historical documents under `docs/archive/pre-v1/`**
...
- [x] **Step 3: Keep active ADRs by technical decision**
...
- [x] **Step 4: Rewrite README around V1 operation rather than chronology**
...
- [x] **Step 5: Update repository archive and release rules**
...
- [x] **Step 6: Validate internal Markdown links and active-name hygiene**
...
- [x] **Step 7: Commit documentation consolidation**
...
- [x] **Step 1: Change Python project metadata to `1.0.0`**
...
- [x] **Step 2: Revalidate the derived container lock**
...
- [x] **Step 3: Run package metadata and candidate-input tests without producing release artifacts**
...
- [x] **Step 4: Commit all build inputs before producing artifacts**

Commit message: `release: freeze v1.0.0 candidate inputs`

- [x] **Step 5: Record the immutable candidate SHA without changing the tree**
...
- [x] **Step 3: Preserve the firmware artifact produced by the complete gate**
...
- [x] **Step 4: Save the Windows candidate evidence outside Git**
...
- [x] **Step 5: Run simulator operator verification with the native Core**
...
- [x] **Step 6: Verify offline export independently of Core**
...
- [x] **Step 7: Run hygiene and sensitive-data checks**
...
- [x] **Step 8: Create the release manifest and record Windows results**
...
- [x] **Step 9: Commit documentation-only Windows evidence**
...
- [x] **Step 1: Synchronize the exact candidate source to Jetson**
...
- [x] **Step 2: Run the Jetson offline aggregate gate**
...
- [x] **Step 3: Save the two candidate container evidence**
...
- [x] **Step 4: Stop before any Bridge start and verify host device identity**
...
- [x] **Step 5: Stop at the physical hardware gate**
...
- [x] **Step 6: Validate Compose and start the two services from the already-built ARM64 candidate image**
...
- [x] **Step 7: Run one bounded Pulse=1 real capture**
...
- [x] **Step 8: Record Jetson evidence**
...
- [x] **Step 9: Commit Jetson acceptance evidence from the release branch**
...
- [x] **Step 1: Audit every cleanup target for unique local content**
...
- [x] **Step 2: Inspect all Junctions before removal**
...
- [x] **Step 3: Remove obsolete worktrees with the appropriate mechanism**
...
- [x] **Step 4: Clean generated caches after evidence preservation**
...
- [x] **Step 5: Re-run final repository checks**
...
- [x] **Step 6: Integrate through a normal merge and push**

Review the branch as a whole, merge it into `main` without rewriting history, push `main`, and verify local/remote SHA equality.

- [x] **Step 7: Prove the merged main preserves the accepted runtime/build inputs and record the checkout-rule exception**

On the clean, merged `main`, before creating a tag, run the original strict check:

```powershell
git diff --exit-code $env:V1_CANDIDATE_SHA HEAD -- . ':(exclude)docs/**'
```

The strict check identified only `.gitattributes`. Review that diff separately, then rerun while excluding `.gitattributes`; expected: no output and exit code 0. The exception only pins the already-tracked container lock bytes to LF in Windows checkouts. All project metadata, dependencies, deployment inputs, runtime source, tests, scripts, protocol, and firmware remain identical to the accepted artifact candidate.

- [x] **Step 8: Create and push the annotated release tag**
...
- [x] **Step 9: Assign final local image tags without rebuilding**
...
- [x] **Step 10: Synchronize final main and tag to Jetson**
...
- [x] **Step 11: Remove the completed release worktree**
...
- [x] **Step 12: Report the frozen release**
...
- The manifest records `V1_CANDIDATE_SHA` as the artifact-build SHA and `v1.0.0` as the final release tag; the final tree differs from the candidate only in acceptance/release documentation and the documented `.gitattributes` LF checkout rule.
